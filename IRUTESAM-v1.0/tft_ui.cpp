...
// ── Backlight PWM — ESP32 Core 3.x ──────────────────────────
// 5 kHz is above the audible range → no coil whine / flicker.
// 8-bit resolution gives 256 steps (0 = off, 255 = full bright).
// Core 3.x API: ledcAttach(pin, freq, res) — no channel number.
#define BL_PWM_FREQ   5000   // Hz
#define BL_PWM_RES       8   // bits (0–255)

// Preset duty values for setBrightnessPreset(0..4)
//   index:   0     1     2     3     4
//   level: 10%   20%   50%   80%  100%
static const uint8_t BL_PRESETS[5] = { 26, 51, 128, 204, 255 };
...
void tftInit() {
    // ── Backlight PWM — must run before SPI / tft.init() ────
    // ESP32 Core 3.x: ledcAttach(pin, freq, resolution)
    // No channel number — the driver assigns one automatically.
    ledcAttach(TFT_BL_PIN, BL_PWM_FREQ, BL_PWM_RES);
    ledcWrite(TFT_BL_PIN, 255);    // start at 100% brightness
...
bool checkDialogTouch(bool &yesSelected) {
    ...
    // add tolerance (fix missed touches)
    int margin = 12;

    if (tx >= 38 - margin && tx <= 110 + margin &&
    ty >= 160 - margin && ty <= 182 + margin) {
        yesSelected = true;
        return true;
    }
    if (tx >= 130 - margin && tx <= 202 + margin &&
    ty >= 160 - margin && ty <= 182 + margin) {
        yesSelected = false;
        return true;
    }
...
void updateNFCEmulatePulse(bool pulse) {
    // status text area
    tft.fillRect(40, 112, 160, 12, DV_BG);
    drawCenteredText(pulse ? "BROADCASTING..." : "STANDBY...", 115,
                     pulse ? DV_CYAN : DV_CYAN_DIM, 1);

    // center pulse dot only
    int cx = 120, cy = 178;
    tft.fillCircle(cx, cy, 6, pulse ? DV_PURPLE : DV_CYAN_DIM);
}
...
void drawAbout() {
    ...
    tft.setCursor(6, 136); tft.print("NFC: PN532 UART");
...
// ── Brightness preset ────────────────────────────────────────
// Applies one of 5 fixed levels to the backlight via PWM.
// preset 0 = 10%, 1 = 20%, 2 = 50%, 3 = 80%, 4 = 100%
void setBrightnessPreset(uint8_t preset) {
    if (preset >= 5) preset = 4;                 // clamp — never go out of range
    ledcWrite(TFT_BL_PIN, BL_PRESETS[preset]);   // write duty to GPIO21
}