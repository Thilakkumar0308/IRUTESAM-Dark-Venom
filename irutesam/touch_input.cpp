...
TouchEvent touchPoll() {
    ...
    if (!_physTouched) {
        ...
        if (p.z < TOUCH_Z_MIN || p.z > TOUCH_Z_MAX) return TOUCH_NONE;

        int x = constrain(map(p.x, TS_MINX, TS_MAXX, 0, SCREEN_W - 1), 0, SCREEN_W - 1);
        int y = constrain(map(p.y, TS_MINY, TS_MAXY, 0, SCREEN_H - 1), 0, SCREEN_H - 1);
